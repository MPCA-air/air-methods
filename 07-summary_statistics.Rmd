...
Click the button below to view code to perform normality test.
...
This web tool will allow visual inspection of air toxics data by year, site and pollutants - [https://mpca-pahs.shinyapps.io/NormViz/](https://mpca-pahs.shinyapps.io/NormViz/).

<br>


Click the button below to view the code for the shiny tool above.
...
Click the button below to view the bootstrapping code.
...
Click the button below to view the code for calculating _UCLs_.

<div class="toggle"><button class = "btn_code">Show __R__ code</button>

```{r confidence, eval=F, warning=F, message=F, echo=F}
...
data = data %>% filter(AQSID == 270370020) %>% mutate(`Monitoring Site` = AQSID, PollutantGroup = Pollutant, Censored = Result < MDL, Result = ifelse(Censored, MDL, Result) )
