...
<br> Create function to test for missing concentration values.
...
<br> Use the `missing_conc()` function to search for observations with missing concentrations.
<div class="fold s o">
```{r eval=T, message=F, warning=F}
...
<br> Remove the blank, NULL, `-999`, and missing values from the data set.
<div class="fold s o">
```{r eval=T, message=F, warning=F}
...
<br> Create similar functions to test for missing dates, site IDs, detection limits, and parameter codes.
...
<br> To apply all these functions at once, use the following script.

<div class="fold s o">
```{r eval=T, message=F, warning=F}
  
# Create new TRUE/FALSE columns labeling each result as missing or not
# Filter to remove any rows with a missing value
data <- data %>% 
        rowwise() %>% 
