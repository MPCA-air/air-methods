...
- \@ref(sticky)   __Check for machine errors (sticking values)__  
- \@ref(quals)    __Evaluate qualified data__     
- \@ref(dups)     __Remove duplicate observations__ 
- \@ref(evalpocs) __Evaluate sites with multiple monitors (POCs)__   
- \@ref(pocs)     __Prioritize observations from multiple monitors (POCs)__   
- \@ref(outs)     __Check for outliers, leaks, and contamination__
...
```{r demo, eval=T, message=F, warning=F, id = "demo", class = "collapse"}
library(dplyr)
...
## Machine errors, sticking values, and sample leaks {#sticky}  
...
```r
data %>% group_by(param_code, cas, aqs_id, conc) %>% summarize(count = n())
...
## Prioritize multiple air monitors (POCs) {#pocs}  
...
## Outliers, leaks, and contamination {#outs}  
...
# Value should not be 20% below background
...
Reference: https://stackoverflow.com/questions/37755037/how-to-add-code-folding-to-output-chunks-in-rmarkdown-html-documents/37839683#37839683
