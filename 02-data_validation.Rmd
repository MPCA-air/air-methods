...
Environmental monitoring data can be subject to error. Three important errors to look out for are:

  1. Loss of sample (leaks).
  2. Instrument issues such as sequential repeated results.
  3. Extreme values.


Validation includes steps to:

- \@ref(leaks)    __Identify leaks and sample contamination__
- \@ref(sticky)   __Check for instrument errors (sticking values)__ 
- \@ref(outs)     __Check for outliers__


<br> __Recommended steps__

1. Identify exceptionally high values.
1. Identify 3 sequential identical values (sticking).
...
- Recommendation 
- __[75th percentile * 3] for all pollutants__

Possible Sample loss issues (leak)-by site and pollutant
...
o	I tried taking the mean of 5 sequential values, and then the 5 sequential values after the first 5. Then, if the second set of means was 1.5 times less than the first mean I flagged the values for a check.

o   Instrument issues
  Check for 3 sequential duplicated values.
 
o   Goals are: 1. Simple to run, 2. To find extreme values that may be in error or from contamination, 3. Find 2 repeating values that are exactly the same, 4. Find low values where this is not common., 5. Do this in a routine way [identify frequency of validation checks].
