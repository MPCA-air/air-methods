...
```{r week_day_chart, echo = F, eval=F}
...
ggplot(df, aes(time, obs)) + geom_line(linetype = "dashed") +
  geom_point(aes(color = weekdays)) + guides(color = guide_legend(title = "Weekday"))
...
```{r hour_day_chart, echo = F, eval=F}

# Sample data
df <- data_frame(time = seq(ymd_hms("2016-01-01 12:00:00"), ymd_hms("2018-02-01 12:00:00"), 1), 
                 obs  = sample(25:110, 763, replace = T)) %>% 
       mutate(n       = 1:n(),
              hour = hour(time))
...
ggplot(df, aes(time, obs)) + geom_line(linetype = "dashed") +
  geom_point(aes(color = hour)) + guides(color = guide_legend(title = "Hour of the Day"))
