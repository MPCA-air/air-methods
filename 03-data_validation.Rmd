...
1. Identify potential instrument drift: Find a significant change in variation in the data in the same time frame from one year to the next. This, or a sudden shift in magnitude of signal, could imply that there is a drift or concentrations are only different due to a new calibration or insturment change.
1. Identify extreme values: Find extreme values that may be in error, from contamination, or from a loss in signal.
1. Identify sequential identical values (sticking): Find 4 sequential identical values within the same site and pollutants.
...
The following sections are examples of data validation steps that could be used to identify these issues. 
...
First it's always a good idea to look at the data. If there are a low number of numeric data points the statistical tests that follow may not be advised.

Open the web tool below to chart the measured pollutants at each site.
