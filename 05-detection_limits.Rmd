...
output$detlim <- renderPlot({
  data_sub = filter(data, year==2009, pollutant=="Benzene")
    data_sub = filter(data, year==input$years, pollutant==input$pollutants)
    ...
    ggtitle(paste0("Site Comparison Boxplot with Censored and Non-Censored Values for", input$pollutants, "from", input$years, "data"))
})
})
