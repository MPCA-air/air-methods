...
Performing similar analysis on multiple pollutants at multiple sites over multiple years becomes much easier when data is in a consitant format. MPCA prefers tidy data where each result has its own row in a table. More information on tidy data can be found here: https://cran.r-project.org/web/packages/tidyr/vignettes/tidy-data.html. For monitoring data, each row should contain columns with the monitoring site ID (ASQ ID is best, but other unique identifiers for each site are okay), parameter code (in some cases other identifiers for pollutants may be used such as CAS or the analyte name, but the identifier must be unique to each analyte in the data), POC (for collocated monitors; if there are no collocated monitors in your data, then it should still include a POC column with all values being 1 to make this clear), sample date (yyyymmdd or yyyy-mm-dd format is best, bot other formats can be converted using R's _lubridate_ package), sample start time (hh:mm:ss), sample duration (code for duration or numeric value of duration), result, and null data code. Optional columns may include qualifier codes and method detection limit. Any fields directly associated with one or more of the aforementioned columns should be kept in their own tables whenever possible with a linking field to join to the data. For example, the state, county, city, zip code, site name, site address, and site location are all fields which are directly associated with AQS ID, so if your data has the AQS ID column, perform all analysis based on that AQS ID, then join the site information by AQS ID after all analysis is completed. EPA's AQS format is a recommended format for air monitoring data as it meets all of the requirements for tidy data.

The functions included in this document often group and sort based on these columns. It is therefore necessary to include these columns in your data if you want to use the functions as-is. You may have to rearrange your data depending on what format you receive in it. For example, if you have multiple results in separate column in a row (such as having a column with the result of each site for a parameter and time), then you will want to reorganize the data so each site result has its own row. The _gather_ function in R's tidyverse package works well for this purpose. The _mutate_ function in R's tidyverse package can add and manipulate columns. _rename_, _names_, and __set_names__ can change column names to make data compatable with the functions in this document.
